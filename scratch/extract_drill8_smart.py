...
for p_idx in range(1, len(reader.pages) - 1): # Pages 2 to 64 (index 1 to 63)
    ...
    # Separate options area and question prompts area
    # In Google Forms PDF:
    # Options area comes first, containing scores 1/1 or 0/1 and option lines (lettered A-D or unlettered 4 choices)
    # Then question prompts area comes, where lines end with '*' or form question sentences!
    ...
    for l in filtered:
        ...
        if score_match and not in_q_section:
            ...
            if l == 'Correct answer':
                ...
                # We have finished option blocks, now in question section!
                if curr_block and curr_block['opts']:
                    option_blocks.append(curr_block)
                    curr_block = None
                in_q_section = True
