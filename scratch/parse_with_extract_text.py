...
for p_idx in range(1, len(reader.pages) - 1): # Skip page 0 (cover) and last page (55)
    page_text = reader.pages[p_idx].extract_text()
    
    # Split page_text by lines
    lines = [l.strip() for l in page_text.split('\n') if l.strip()]
    
    # Filter out footer lines and basic info fields
    filtered = []
    for l in lines:
...
for p_num, lines in pages_data[:5]:
    print(f"=== PAGE {p_num} ===")
    for l in lines:
        print("  ", l)
